    ...
    <GovLayout>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/50 overflow-hidden flex flex-col md:flex-row border border-white">
                    <div class="md:w-1/3 bg-[#002B5B] relative flex items-center justify-center p-8">
                        ...
                        <div class="relative z-10 aspect-[3/4] w-full bg-slate-800 rounded-2xl shadow-2xl flex items-center justify-center overflow-hidden border-2 border-white/20">
                            <span class="text-white/20 italic">{{ $t("welcome.director_name") }}</span>
                        </div>
                    </div>
        ...
        <section class="py-20 max-w-7xl mx-auto px-4">
            <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                ...
                <Link :href="route('news.index')" class="btn btn-primary btn-outline border-2 rounded-xl px-8 hover:bg-[#002B5B]">
                    {{ $t("welcome.view_all") }}
                </Link>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                <div v-for="post in latestPosts" :key="post.id" class="group flex flex-col bg-white rounded-3xl transition-all duration-500 hover:-translate-y-2">
                    <div class="relative h-64 w-full overflow-hidden rounded-3xl shadow-lg">
                        <img v-if="post.featured_image" :src="post.featured_image" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div v-else class="h-full w-full bg-slate-200 flex items-center justify-center text-slate-400">No Image</div>
                    ...
                    <div class="pt-6 px-2">
                        ...
                        <div class="flex items-center gap-3 mt-4">
                            <Link :href="route('news.show', post.id)" class="text-sm font-bold text-[#D4AF37] flex items-center gap-1 group/link">
                                Read Article <span class="transition-transform group-hover/link:translate-x-1">→</span>
                            </Link>
                        </div>
                    </div>
                </div>
                                ...
                                <tr v-for="doc in latestDocuments" :key="doc.id" class="group shadow-sm hover:shadow-md transition-all">
                                    ...
                                    <td class="bg-white">
                                        <span class="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-xs font-medium uppercase">
                                            {{ $t(`documents.types.${doc.document_type.toLowerCase().replace(' ', '_')}`) }}
                                        </span>
                                    </td>
                                    <td class="bg-white rounded-r-2xl text-right">
                                        <a :href="'/storage/' + doc.file_path" target="_blank" class="btn btn-circle btn-ghost btn-sm text-primary">
